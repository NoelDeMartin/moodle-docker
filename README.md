...
In order to run Behat tests for the mobile app, you need to install the [local_moodlemobileapp](https://github.com/moodlehq/moodle-local_moodlemobileapp) plugin. Everything else should be the same as running standard Behat tests for Moodle. Make sure to filter tests using the `@app` tag.

For this to work, you need to have your environment configured to serve the `mobileapp` container as well. Set the `MOODLE_DOCKER_APP_PATH` env variable to the app codebase in you file system and `MOODLE_DOCKER_BROWSER` to "chrome". The container assumes that you've already initialized the app calling `npm install` and `npm run setup` locally.
...
This *can* be used for local development of the mobile app, given that the `moodleapp` container serves the app on the 8100 port. However, this is intended to run Behat tests that require interacting with a local Moodle environment. Normal development should be easier calling `npm start` in the host system.
